...
所有目標、測量值、設定與彙總資料都保存在本機 SQLite 資料庫中。LNPM 不會上傳監控資料，只會連線至已設定的探測目標，並每天一次存取公開的 GitHub Releases API 檢查更新。
...
每次 push 與 pull request 都會經過跨平台 CI 工作流程驗證。建立 `v0.1.0` 之類的版本標籤後會啟動發佈工作流程，並在所有平台建置成功後發佈原生套件。
