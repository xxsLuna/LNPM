...
所有目标、测量值、设置和汇总数据均保存在本地 SQLite 数据库中。LNPM 不会上传监控数据，只会连接已配置的探测目标，并每天一次访问公开的 GitHub Releases API 检查更新。
...
每次 push 和 pull request 都会通过跨平台 CI 工作流验证。创建 `v0.1.0` 之类的版本标签后会启动发布工作流，并在所有平台构建成功后发布原生软件包。
