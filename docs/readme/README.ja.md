<div align="center">
  <img src="../assets/lnpm-logo.png" alt="LNPM ロゴ" width="112" />
...
対象、測定値、設定、集計はすべてローカル SQLite データベースに保存されます。LNPM は監視データをアップロードしません。設定された測定先への通信と、1日1回の公開 GitHub Releases API による更新確認だけを行います。
...
すべての push と pull request はクロスプラットフォーム CI で検証されます。`v0.1.0` のようなバージョンタグを作成するとリリースワークフローが開始し、全プラットフォームのビルド成功後にネイティブパッケージを公開します。
