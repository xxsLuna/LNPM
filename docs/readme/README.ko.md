...
모든 대상, 측정값, 설정, 집계 데이터는 로컬 SQLite 데이터베이스에 저장됩니다. LNPM은 모니터링 데이터를 업로드하지 않습니다. 설정된 측정 대상에 연결하고 하루에 한 번 공개 GitHub Releases API에서 업데이트만 확인합니다.
...
모든 push와 pull request는 크로스 플랫폼 CI 워크플로로 검증됩니다. `v0.1.0`과 같은 버전 태그를 만들면 릴리스 워크플로가 시작되고, 모든 플랫폼 빌드가 성공한 뒤 네이티브 패키지를 게시합니다.
